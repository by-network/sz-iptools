module.exports = {
  ...
  getIpData: getIpData,
  getNextIp: getNextIp,
  getPreviousIp: getPreviousIp,
  onSameNetwork: onSameNetwork,
  convertCidrToNetmask: convertCidrToNetmask,
  convertNetmaskToCidr: convertNetmaskToCidr
}
...
/**
 * @name convertNetmaskToCidr
 * @description Converts netmask string, with dotted notation, to cidr prefix.
 * @param {string} str Netmask to convert.
 * @param {function} callback Callback function
 * @author Guilherme Somenzi <guilherme@somenzi.me>
 */
...
/**
 * @name convertCidrToNetmask
 * @description Converts cidr prefix to netmask string, with dotted notation.
 * @param {string|number} str CIDR to convert.
 * @param {function} callback Callback function
 * @author Guilherme Somenzi <guilherme@somenzi.me>
 */
...
/**
 * @name getNextIp
 * @description Gets the next IP address of a network.
 * @param {string} ipWithNetmask IP address with CIDR sufix.
 * @param {function} callback Callback function
 * @author Guilherme Somenzi <guilherme@somenzi.me>
 */
function getNextIp (ipWithNetmask, callback) {
  let currentIp = ipWithNetmask.split('/')[0]
  if(!ipWithNetmask.split('/')[1]) return new Error('To calculate next IP address you must to provide an IP address with netmask.')
  getIPDataWithNetmask(ipWithNetmask, (err, netData) => {
    if (err) return callback(err)
    convertIpToDecimal(currentIp, (err, currentDecimal) => {
      if (err) return callback(err)
      let nextDecimal = currentDecimal + 1
      if (nextDecimal <= netData.higher.decimal) {
        convertDecimalToIp(nextDecimal, (err, nextIp) => {
          if (err) return callback(err)
          return callback(null, nextIp)
        })
      } else {
        return callback(new Error('It is the higher IP of this network.'))
      }
    })
  })
}
...
/**
 * @name getPreviousIp
 * @description Gets the previous IP address of a network.
 * @param {string} ipWithNetmask IP address with CIDR sufix.
 * @param {function} callback Callback function
 * @author Guilherme Somenzi <guilherme@somenzi.me>
 */
function getPreviousIp (ipWithNetmask, callback) {
  let currentIp = ipWithNetmask.split('/')[0]
  if(!ipWithNetmask.split('/')[1]) return new Error('To calculate previous IP address you must to provide an IP address with netmask.')
  getIPDataWithNetmask(ipWithNetmask, (err, netData) => {
    if (err) return callback(err)
    convertIpToDecimal(currentIp, (err, currentDecimal) => {
      if (err) return callback(err)
      let prevDecimal = currentDecimal - 1
      if (prevDecimal >= netData.lower.decimal) {
        convertDecimalToIp(prevDecimal, (err, prevIp) => {
          if (err) return callback(err)
          return callback(null, prevIp)
        })
      } else {
        return callback(new Error('It is the lower IP of this network.'))
      }
    })
  })
}
...
function convertDecimalToIp (decimal, callback) {
  var str = decimal % 256
  for( var i = 3; i > 0; i-- ) {

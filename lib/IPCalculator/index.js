...
function onSameNetwork (ip1, ip2, netmask, callback) {
  ...
  isIPv4(ip1, (err, ip1IsIpv4) => {
    ...
    isIPv4(ip2, (err, ip2IsIpv4) => {
      ...
      getLowerIp(ip1, netmask, (err, lowerIp1) => {
        if (err) return callback(err)
        getHigherIp(ip1, netmask, (err, higherIp1) => {
          if (err) return callback(err)
    ...
    convertNetmaskToCidr(netmask, (err, validCidr) => {
      ...
      convertIpToDecimal(str, (err, ipDecimal) => {
        ...
        convertIpToBinary(str, (err, ipBinary) => {
          ...
          obj.address.binary = ipBinary
          // Get lower ip
          getLowerIp(str.split('/')[0], netmask, (err, lowerIp) => {
            if (err) return callback(err)
            obj.lower = lowerIp
            obj.network.address = lowerIp.string
            // Get higher ip
            getHigherIp(str.split('/')[0], netmask, (err, higherIp) => {
              if (err) return callback(err)
              obj.higher = higherIp
              obj.network.broadcast = higherIp.string
              obj.network.hostsNumber = higherIp.decimal - lowerIp.decimal - 2
